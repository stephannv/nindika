...
module Prices
  class CreateGameEvent < Actor
    ...
    def call
      ...
      create_event(GameEventTypes::PRICE_ADDED) if price_added?
      create_event(GameEventTypes::DISCOUNT) if discount?
      create_event(GameEventTypes::PERMANENT_PRICE_CHANGE) if permanent_price_change?
      create_event(GameEventTypes::PRICE_STATE_CHANGE) if price_state_change?
    end

    private

    def create_event(event_type)
      GameEvents::Create.call(event_type: event_type, game: price.game, price: price)
    end

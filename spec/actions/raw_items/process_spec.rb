...
RSpec.describe RawItems::Process, type: :action do
  ...
  describe '#call' do
    ...
    before do
      ...
      allow(Nintendo::AlgoliaDataAdapter).to receive(:adapt).with(raw_item.data).and_return(adapted_data)
    end
    ...
    context 'when some error happens on development environment' do
      ...
      before do
        ...
        allow(Nintendo::AlgoliaDataAdapter).to receive(:adapt).and_raise(error)
      end

...
RSpec.describe GameEventDataBuilder, type: :lib do
  subject(:data) { described_class.build(event_type: event_type, game: game, price: price) }

  let(:price) { nil }

  context 'when event type is GAME_ADDED' do
    let(:event_type) { GameEventTypes::GAME_ADDED }
    let(:game) { build(:game) }

    it 'has release_date attribute' do
      expect(data[:release_date]).to eq game.release_date_text
    end
  end

  context 'when event type is PRICE_ADDED' do
    let(:event_type) { GameEventTypes::PRICE_ADDED }
    let(:game) { build(:game) }
  ...
  context 'when event type is DISCOUNT' do
    let(:event_type) { GameEventTypes::DISCOUNT }
    let(:game) { build(:game) }
    let(:price) { build(:price, :with_discount) }
  ...
  context 'when event type is PERMANENT_PRICE_CHANGE' do
    let(:event_type) { GameEventTypes::PERMANENT_PRICE_CHANGE }
  ...
  context 'when event type is PRICE_STATE_CHANGE' do
    let(:event_type) { GameEventTypes::PRICE_STATE_CHANGE }
    let(:game) { build(:game) }
    let(:price) { build(:price) }

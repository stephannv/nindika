...
RSpec.describe Prices::CreateGameEvent, type: :actions do
  describe 'Inputs' do
  ...
  describe '#call' do
    ...
    it 'can create more than one event' do
      ...
      expect { result }.to change(price.game.events, :count).by(2)
    end

    context 'when price or state didn`t change' do
      before { price.reload }

      it 'doesn`t create game event' do
    ...
    context 'when price is newly created' do
      it 'creates PRICE_ADDED game event' do
        expect(GameEvents::Create).to receive(:call)
          .with(event_type: GameEventTypes::PRICE_ADDED, game: price.game, price: price)
    ...
    context 'when existing price receives discount' do
      ...
      it 'creates DISCOUNT game event' do
        expect(GameEvents::Create).to receive(:call)
          .with(event_type: GameEventTypes::DISCOUNT, game: price.game, price: price)
    ...
    context 'when base price changes' do
      ...
      it 'creates PERMANENT_PRICE_CHANGE game event' do
        expect(GameEvents::Create).to receive(:call)
          .with(event_type: GameEventTypes::PERMANENT_PRICE_CHANGE, game: price.game, price: price)
    ...
    context 'when price state changes to unavailable' do
      ...
      it 'creates PRICE_STATE_CHANGE game event' do
        expect(GameEvents::Create).to receive(:call)
          .with(event_type: GameEventTypes::PRICE_STATE_CHANGE, game: price.game, price: price)
